...
module Tincan
  ...
  class Package
    class Activity
      include XML::Mapping
      text_node :activity_id, "@id"
      text_node :activity_type, "@type"
      text_node :name, "name", :class => String, :default_value => ""
      text_node :description, "description", :class => String, :default_value => ""
      text_node :launch, "launch", :class => String, :default_value => ""
    end

    class Manifest
      include XML::Mapping
      array_node :activities, "activities", "activity", :class => Activity
    end

    attr_accessor :name       # Name of the package.
    attr_accessor :manifest   # An instance of +Tincan::Manifest+.
    ...
    def initialize(filename, options = {}, &block)
      ...
      if exists?('tincan.xml')
        Rails.logger.info @path
        @manifest = Manifest.load_from_file File.join(@path, 'tincan.xml')
      else
        raise InvalidPackage, "#{File.basename(@package)}: no tincan.xml, maybe not Tincan compatible?"
      end
